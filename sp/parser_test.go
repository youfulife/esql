...
func TestParser_ParseQuery(t *testing.T) {
	...
	q, err := sp.NewParser(strings.NewReader(s)).ParseQuery()
	if err != nil {
...
func TestParser_ParseQuery_Empty(t *testing.T) {
	q, err := sp.NewParser(strings.NewReader(``)).ParseQuery()
	if err != nil {
		t.Fatalf("unexpected error: %s", err)
...
func TestParser_ParseQuery_ParseError(t *testing.T) {
	_, err := sp.NewParser(strings.NewReader(`SELECT`)).ParseQuery()
...
func TestParser_ParseQuery_NoSemicolon(t *testing.T) {
	_, err := sp.NewParser(strings.NewReader(`CREATE DATABASE foo CREATE DATABASE bar`)).ParseQuery()
...
func TestParser_ParseStatement(t *testing.T) {
	...
	var tests = []struct {
		...
		// SELECT * statement
		{
			...
			stmt: &sp.SelectStatement{
				...
				Sources: []sp.Source{&sp.Measurement{Name: "myseries"}},
			},
		},
		{
		...
		// SELECT statement
		{
			...
			stmt: &sp.SelectStatement{
				IsRawQuery: false,
				Fields: []*sp.Field{
					{Expr: &sp.Call{Name: "mean", Args: []sp.Expr{&sp.VarRef{Val: "field1"}}}},
					{Expr: &sp.Call{Name: "sum", Args: []sp.Expr{&sp.VarRef{Val: "field2"}}}},
					{Expr: &sp.Call{Name: "count", Args: []sp.Expr{&sp.VarRef{Val: "field3"}}}, Alias: "field_x"},
				},
				Sources: []sp.Source{&sp.Measurement{Name: "myseries"}},
				Condition: &sp.BinaryExpr{
					...
					LHS: &sp.BinaryExpr{
						Op:  sp.EQ,
						LHS: &sp.VarRef{Val: "host"},
						RHS: &sp.StringLiteral{Val: "hosta.influxdb.org"},
					},
					RHS: &sp.BinaryExpr{
						Op:  sp.GT,
						LHS: &sp.VarRef{Val: "time"},
						RHS: &sp.StringLiteral{Val: now.UTC().Format(time.RFC3339Nano)},
					},
				},
				Dimensions: []*sp.Dimension{{Expr: &sp.Call{Name: "time", Args: []sp.Expr{&sp.DurationLiteral{Val: 10 * time.Hour}}}}},
		...
		},
		{
			...
			stmt: &sp.SelectStatement{
				IsRawQuery: true,
				Fields: []*sp.Field{
					{Expr: &sp.VarRef{Val: "foo.bar.baz"}, Alias: "foo"},
				},
				Sources: []sp.Source{&sp.Measurement{Name: "myseries"}},
			},
		},
		{
			...
			stmt: &sp.SelectStatement{
				...
				Sources: []sp.Source{&sp.Measurement{Name: "foo"}},
			},
		},
		...
		// sample
		{
			...
			stmt: &sp.SelectStatement{
				IsRawQuery: false,
				Fields: []*sp.Field{
					{Expr: &sp.Call{Name: "sample", Args: []sp.Expr{&sp.VarRef{Val: "field1"}, &sp.IntegerLiteral{Val: 100}}}},
				},
				Sources: []sp.Source{&sp.Measurement{Name: "myseries"}},
			},
		},
		...
		// derivative
		{
			...
			stmt: &sp.SelectStatement{
				IsRawQuery: false,
				Fields: []*sp.Field{
					{Expr: &sp.Call{Name: "derivative", Args: []sp.Expr{&sp.VarRef{Val: "field1"}, &sp.DurationLiteral{Val: time.Hour}}}},
				},
				Sources: []sp.Source{&sp.Measurement{Name: "myseries"}},
			},
		},
				...
				Fields: []*sp.Field{
					{
						Expr: &sp.BinaryExpr{
							LHS: &sp.Call{
								Name: "derivative",
								Args: []sp.Expr{
									&sp.VarRef{Val: "field1"},
									&sp.DurationLiteral{Val: time.Hour},
								},
							},
							RHS: &sp.Call{
								Name: "derivative",
								Args: []sp.Expr{
									&sp.VarRef{Val: "field2"},
									&sp.DurationLiteral{Val: time.Hour},
								},
							},
							Op: sp.DIV,
		...
		// difference
		{
			...
			stmt: &sp.SelectStatement{
				IsRawQuery: false,
				Fields: []*sp.Field{
					{Expr: &sp.Call{Name: "difference", Args: []sp.Expr{&sp.VarRef{Val: "field1"}}}},
				},
				Sources: []sp.Source{&sp.Measurement{Name: "myseries"}},
			},
		},
				...
				Dimensions: []*sp.Dimension{
					{
						Expr: &sp.Call{
							Name: "time",
							Args: []sp.Expr{
								&sp.DurationLiteral{Val: time.Minute},
							},
						},
					},
				},
		...
		// moving_average
		{
			...
			stmt: &sp.SelectStatement{
				IsRawQuery: false,
				Fields: []*sp.Field{
					{Expr: &sp.Call{Name: "moving_average", Args: []sp.Expr{&sp.VarRef{Val: "field1"}, &sp.IntegerLiteral{Val: 3}}}},
				},
				Sources: []sp.Source{&sp.Measurement{Name: "myseries"}},
			},
		},
				...
				Dimensions: []*sp.Dimension{
					{
						Expr: &sp.Call{
							Name: "time",
							Args: []sp.Expr{
								&sp.DurationLiteral{Val: 1 * time.Hour},
							},
						},
					},
				},
				...
				Fields: []*sp.Field{
					{
						Expr: &sp.Call{
							...
							Args: []sp.Expr{
								...
								&sp.IntegerLiteral{Val: 4},
								&sp.IntegerLiteral{Val: 5},
							},
						},
					},
				},
		...
		// SELECT statement (lowercase)
		{
			...
			stmt: &sp.SelectStatement{
				IsRawQuery: true,
				Fields:     []*sp.Field{{Expr: &sp.VarRef{Val: "my_field"}}},
				Sources:    []sp.Source{&sp.Measurement{Name: "myseries"}},
			},
		},
		...
		// SELECT statement (lowercase) with quoted field
		{
			...
			stmt: &sp.SelectStatement{
				IsRawQuery: true,
				Fields:     []*sp.Field{{Expr: &sp.StringLiteral{Val: "my_field"}}},
				Sources:    []sp.Source{&sp.Measurement{Name: "myseries"}},
			},
		},
		...
		// SELECT * FROM cpu WHERE host = 'serverC' AND region =~ /.*west.*/
		{
			...
			stmt: &sp.SelectStatement{
				...
				Sources:    []sp.Source{&sp.Measurement{Name: "cpu"}},
				Condition: &sp.BinaryExpr{
					Op: sp.AND,
					LHS: &sp.BinaryExpr{
						Op:  sp.EQ,
		...
		// select percentile statements
		{
			...
			stmt: &sp.SelectStatement{
				IsRawQuery: false,
				Fields: []*sp.Field{
					{Expr: &sp.Call{Name: "percentile", Args: []sp.Expr{&sp.VarRef{Val: "field1"}, &sp.NumberLiteral{Val: 2.0}}}},
				},
				Sources: []sp.Source{&sp.Measurement{Name: "cpu"}},
			},
		},
		...
		// select top statements
		{
			...
			stmt: &sp.SelectStatement{
				IsRawQuery: false,
				Fields: []*sp.Field{
					{Expr: &sp.Call{Name: "top", Args: []sp.Expr{&sp.VarRef{Val: "field1"}, &sp.IntegerLiteral{Val: 2}}}},
				},
				Sources: []sp.Source{&sp.Measurement{Name: "cpu"}},
			},
		},
...

		{
			...
			stmt: &sp.SelectStatement{
				IsRawQuery: false,
				Fields: []*sp.Field{
					{Expr: &sp.Call{Name: "top", Args: []sp.Expr{&sp.VarRef{Val: "field1"}, &sp.VarRef{Val: "tag1"}, &sp.IntegerLiteral{Val: 2}}}},
					{Expr: &sp.VarRef{Val: "tag1"}},
				},
				Sources: []sp.Source{&sp.Measurement{Name: "cpu"}},
			},
		...
		// select distinct statements
		{
			...
			stmt: &sp.SelectStatement{
				IsRawQuery: false,
				Fields: []*sp.Field{
					{Expr: &sp.Call{Name: "distinct", Args: []sp.Expr{&sp.VarRef{Val: "field1"}}}},
				},
				Sources: []sp.Source{&sp.Measurement{Name: "cpu"}},
			},
		},

		{
			...
			stmt: &sp.SelectStatement{
				IsRawQuery: true,
				Fields: []*sp.Field{
					{Expr: &sp.Distinct{Val: "field2"}},
				},
				Sources: []sp.Source{&sp.Measurement{Name: "network"}},
			},
		},

		{
			...
			stmt: &sp.SelectStatement{
				IsRawQuery: false,
				Fields: []*sp.Field{
					{Expr: &sp.Call{Name: "count", Args: []sp.Expr{&sp.Distinct{Val: "field3"}}}},
				},
				Sources: []sp.Source{&sp.Measurement{Name: "metrics"}},
			},
		},

		{
			...
			stmt: &sp.SelectStatement{
				...
				Fields: []*sp.Field{
					{Expr: &sp.Call{Name: "count", Args: []sp.Expr{&sp.Distinct{Val: "field3"}}}},
					{Expr: &sp.Call{Name: "sum", Args: []sp.Expr{&sp.VarRef{Val: "field4"}}}},
				},
				Sources: []sp.Source{&sp.Measurement{Name: "metrics"}},
			},
		},

		{
			...
			stmt: &sp.SelectStatement{
				IsRawQuery: false,
				Fields: []*sp.Field{
					{Expr: &sp.Call{Name: "count", Args: []sp.Expr{&sp.Call{Name: "distinct", Args: []sp.Expr{&sp.VarRef{Val: "field3"}}}}}},
					{Expr: &sp.Call{Name: "sum", Args: []sp.Expr{&sp.VarRef{Val: "field4"}}}},
				},
				Sources: []sp.Source{&sp.Measurement{Name: "metrics"}},
			},
		...
		// SELECT * FROM /<regex>/
		{
			...
			stmt: &sp.SelectStatement{
				IsRawQuery: true,
				Fields:     []*sp.Field{{Expr: &sp.Wildcard{}}},
				Sources: []sp.Source{&sp.Measurement{
					Regex: &sp.RegexLiteral{Val: regexp.MustCompile("cpu.*")}},
				},
			},
		},
		...
		// SELECT statement with group by
		{
			...
			stmt: &sp.SelectStatement{
				IsRawQuery: false,
				Fields: []*sp.Field{
					{Expr: &sp.Call{Name: "sum", Args: []sp.Expr{&sp.VarRef{Val: "value"}}}},
				},
				Sources:    []sp.Source{&sp.Measurement{Name: "kbps"}},
				Dimensions: []*sp.Dimension{{Expr: &sp.Call{Name: "time", Args: []sp.Expr{&sp.DurationLiteral{Val: 60 * time.Second}}}}},
				Condition: &sp.BinaryExpr{ // 1
					Op: sp.AND,
					LHS: &sp.BinaryExpr{ // 2
						Op: sp.AND,
						LHS: &sp.BinaryExpr{ //3
							...
							RHS: &sp.BinaryExpr{
								Op:  sp.SUB,
								LHS: &sp.Call{Name: "now"},
								RHS: &sp.DurationLiteral{Val: mustParseDuration("120s")},
							},
						},
						RHS: &sp.BinaryExpr{
							Op:  sp.EQ,
							LHS: &sp.VarRef{Val: "deliveryservice"},
							RHS: &sp.StringLiteral{Val: "steam-dns"},
						},
					},
					RHS: &sp.BinaryExpr{
						Op:  sp.EQ,
						LHS: &sp.VarRef{Val: "cachegroup"},
						RHS: &sp.StringLiteral{Val: "total"},
					},
				},
			},
		},
		// SELECT statement with group by and multi digit duration (prevent regression from #731://github.com/influxdata/influxdb/pull/7316)
		{
			...
			stmt: &sp.SelectStatement{
				Fields: []*sp.Field{{
					Expr: &sp.Call{
						Name: "count",
						Args: []sp.Expr{&sp.VarRef{Val: "value"}}}}},
				Sources: []sp.Source{&sp.Measurement{Name: "cpu"}},
				Condition: &sp.BinaryExpr{
					Op:  sp.LT,
					LHS: &sp.VarRef{Val: "time"},
				...
				Dimensions: []*sp.Dimension{{Expr: &sp.Call{Name: "time", Args: []sp.Expr{&sp.DurationLiteral{Val: 500 * time.Millisecond}}}}},
			},
		},
		...
		// SELECT statement with fill
		{
			...
			stmt: &sp.SelectStatement{
				...
				Dimensions: []*sp.Dimension{{Expr: &sp.Call{Name: "time", Args: []sp.Expr{&sp.DurationLiteral{Val: 5 * time.Minute}}}}},
				Fill:       sp.NumberFill,
				FillValue:  int64(1),
			},
		},
		...
		// SELECT statement with a bound parameter
		{
			...
			stmt: &sp.SelectStatement{
				IsRawQuery: true,
				Fields: []*sp.Field{{
					Expr: &sp.VarRef{Val: "value"}}},
				Sources: []sp.Source{&sp.Measurement{Name: "cpu"}},
				Condition: &sp.BinaryExpr{
					Op:  sp.GT,
					LHS: &sp.VarRef{Val: "value"},
		...
		//	stmt: &sp.DeleteStatement{
		//		Source: &sp.Measurement{Name: "myseries"},
		//		Condition: &sp.BinaryExpr{
		//			Op:  sp.EQ,
		//			LHS: &sp.VarRef{Val: "host"},
		//			RHS: &sp.StringLiteral{Val: "hosta.influxdb.org"},
		...
		// SHOW GRANTS
		{
			...
			stmt: &sp.ShowGrantsForUserStatement{Name: "jdoe"},
		},

		// SHOW DATABASES
		{
			s:    `SHOW DATABASES`,
			stmt: &sp.ShowDatabasesStatement{},
		},

		// SHOW SERIES statement
		{
			s:    `SHOW SERIES`,
			stmt: &sp.ShowSeriesStatement{},
		},
		...
		// SHOW SERIES FROM /<regex>/
		{
			...
			stmt: &sp.ShowSeriesStatement{
				Sources: []sp.Source{
					&sp.Measurement{
						Regex: &sp.RegexLiteral{Val: regexp.MustCompile(`[cg]pu`)},
					},
				},
			},
		},
		...
		// SHOW SERIES with OFFSET 0
		{
			...
			stmt: &sp.ShowSeriesStatement{Offset: 0},
		},

		// SHOW SERIES with LIMIT 2 OFFSET 0
		{
			s:    `SHOW SERIES LIMIT 2 OFFSET 0`,
			stmt: &sp.ShowSeriesStatement{Offset: 0, Limit: 2},
		},
		...
		// SHOW SERIES WHERE with ORDER BY and LIMIT
		{
			...
			stmt: &sp.ShowSeriesStatement{
				Condition: &sp.BinaryExpr{
					Op:  sp.EQ,
					LHS: &sp.VarRef{Val: "region"},
					RHS: &sp.StringLiteral{Val: "order by desc"},
				},
				SortFields: []*sp.SortField{
					&sp.SortField{Ascending: false},
					&sp.SortField{Name: "field1", Ascending: true},
					&sp.SortField{Name: "field2"},
				},
				Limit: 10,
			},
		},
		...
		// SHOW MEASUREMENTS WHERE with ORDER BY and LIMIT
		{
			...
			stmt: &sp.ShowMeasurementsStatement{
				Condition: &sp.BinaryExpr{
					Op:  sp.EQ,
					LHS: &sp.VarRef{Val: "region"},
					RHS: &sp.StringLiteral{Val: "uswest"},
				},
				SortFields: []*sp.SortField{
					{Ascending: true},
					{Name: "field1"},
		...
		// SHOW MEASUREMENTS WITH MEASUREMENT = cpu
		{
			...
			stmt: &sp.ShowMeasurementsStatement{
				Source: &sp.Measurement{Name: "cpu"},
			},
		},
		...
		{
			s:    `SHOW QUERIES`,
			stmt: &sp.ShowQueriesStatement{},
		},

		// KILL QUERY 4
		{
			s: `KILL QUERY 4`,
			stmt: &sp.KillQueryStatement{
				QueryID: 4,
			},
		},
		...
		// SHOW RETENTION POLICIES
		{
			...
			stmt: &sp.ShowRetentionPoliciesStatement{},
		},
		...
		// SHOW TAG KEYS
		{
			...
			stmt: &sp.ShowTagKeysStatement{
				Sources: []sp.Source{&sp.Measurement{Name: "src"}},
			},
		},
		...
		// SHOW TAG VALUES FROM ... WITH KEY = ...
		{
			...
			stmt: &sp.ShowTagValuesStatement{
				Sources:    []sp.Source{&sp.Measurement{Name: "src"}},
				Op:         sp.EQ,
				TagKeyExpr: &sp.StringLiteral{Val: "region"},
				Condition: &sp.BinaryExpr{
					Op:  sp.EQ,
					LHS: &sp.VarRef{Val: "region"},
					RHS: &sp.StringLiteral{Val: "uswest"},
		...
		// SHOW TAG VALUES FROM ... WITH KEY IN...
		{
			...
			stmt: &sp.ShowTagValuesStatement{
				Sources:    []sp.Source{&sp.Measurement{Name: "cpu"}},
				Op:         sp.IN,
				TagKeyExpr: &sp.ListLiteral{Vals: []string{"region", "host"}},
				Condition: &sp.BinaryExpr{
					Op:  sp.EQ,
					LHS: &sp.VarRef{Val: "region"},
					RHS: &sp.StringLiteral{Val: "uswest"},
		...
		// SHOW TAG VALUES ... AND TAG KEY =
		{
			...
			stmt: &sp.ShowTagValuesStatement{
				Sources:    []sp.Source{&sp.Measurement{Name: "cpu"}},
				Op:         sp.IN,
				TagKeyExpr: &sp.ListLiteral{Vals: []string{"region", "service", "host"}},
				Condition: &sp.BinaryExpr{
					Op:  sp.EQ,
					LHS: &sp.VarRef{Val: "region"},
					RHS: &sp.StringLiteral{Val: "uswest"},
		...
		// SHOW TAG VALUES WITH KEY = ...
		{
			...
			stmt: &sp.ShowTagValuesStatement{
				Op:         sp.EQ,
				TagKeyExpr: &sp.StringLiteral{Val: "host"},
				Condition: &sp.BinaryExpr{
					Op:  sp.EQ,
					LHS: &sp.VarRef{Val: "region"},
					RHS: &sp.StringLiteral{Val: "uswest"},
		...
		// SHOW TAG VALUES WITH KEY =~ /<regex>/
		{
			...
			stmt: &sp.ShowTagValuesStatement{
				Op:         sp.EQREGEX,
				TagKeyExpr: &sp.RegexLiteral{Val: regexp.MustCompile(`(host|region)`)},
			},
		},
		...
		// SHOW FIELD KEYS
		{
			...
			stmt: &sp.ShowFieldKeysStatement{
				Sources: []sp.Source{&sp.Measurement{Name: "src"}},
				SortFields: []*sp.SortField{
					{Ascending: true},
					{Name: "field1"},
		...
		{
			s:    `DELETE FROM src`,
			stmt: &sp.DeleteSeriesStatement{Sources: []sp.Source{&sp.Measurement{Name: "src"}}},
		},
		{
			s: `DELETE WHERE host = 'hosta.influxdb.org'`,
			stmt: &sp.DeleteSeriesStatement{
				Condition: &sp.BinaryExpr{
					Op:  sp.EQ,
					LHS: &sp.VarRef{Val: "host"},
					RHS: &sp.StringLiteral{Val: "hosta.influxdb.org"},
		...
		// DROP SERIES statement
		{
			...
			stmt: &sp.DropSeriesStatement{Sources: []sp.Source{&sp.Measurement{Name: "src"}}},
		},
		{
			s: `DROP SERIES WHERE host = 'hosta.influxdb.org'`,
			stmt: &sp.DropSeriesStatement{
				Condition: &sp.BinaryExpr{
					Op:  sp.EQ,
					LHS: &sp.VarRef{Val: "host"},
					RHS: &sp.StringLiteral{Val: "hosta.influxdb.org"},
		...
		// SHOW CONTINUOUS QUERIES statement
		{
			...
			stmt: &sp.ShowContinuousQueriesStatement{},
		},
		...
		// CREATE CONTINUOUS QUERY ... INTO <measurement>
		{
			...
			stmt: &sp.CreateContinuousQueryStatement{
				Name:     "myquery",
				Database: "testdb",
				Source: &sp.SelectStatement{
					Fields:  []*sp.Field{{Expr: &sp.Call{Name: "count", Args: []sp.Expr{&sp.VarRef{Val: "field1"}}}}},
					Target:  &sp.Target{Measurement: &sp.Measurement{Name: "measure1", IsTarget: true}},
					Sources: []sp.Source{&sp.Measurement{Name: "myseries"}},
					Dimensions: []*sp.Dimension{
						{
							Expr: &sp.Call{
								Name: "time",
								Args: []sp.Expr{
									&sp.DurationLiteral{Val: 5 * time.Minute},
								},
							},
						},
					},
...

		{
			...
			stmt: &sp.CreateContinuousQueryStatement{
				...
				Source: &sp.SelectStatement{
					...
					Fields:     []*sp.Field{{Expr: &sp.Wildcard{}}},
					Target:     &sp.Target{Measurement: &sp.Measurement{Name: "measure1", IsTarget: true}},
					Sources:    []sp.Source{&sp.Measurement{Name: "cpu_load_short"}},
				},
			},
		},
		...
		// CREATE CONTINUOUS QUERY ... INTO <retention-policy>.<measurement>
		{
			...
			stmt: &sp.CreateContinuousQueryStatement{
				...
				Source: &sp.SelectStatement{
					Fields: []*sp.Field{{Expr: &sp.Call{Name: "count", Args: []sp.Expr{&sp.VarRef{Val: "field1"}}}}},
					Target: &sp.Target{
						Measurement: &sp.Measurement{RetentionPolicy: "1h.policy1", Name: "cpu.load", IsTarget: true},
					},
					Sources: []sp.Source{&sp.Measurement{Name: "myseries"}},
					Dimensions: []*sp.Dimension{
						{
		...
		// CREATE CONTINUOUS QUERY for non-aggregate SELECT stmts
		{
			...
			stmt: &sp.CreateContinuousQueryStatement{
				...
				Source: &sp.SelectStatement{
					IsRawQuery: true,
					Fields:     []*sp.Field{{Expr: &sp.VarRef{Val: "value"}}},
					Target: &sp.Target{
						Measurement: &sp.Measurement{RetentionPolicy: "policy1", Name: "value", IsTarget: true},
					},
					Sources: []sp.Source{&sp.Measurement{Name: "myseries"}},
				},
			},
		...
		// CREATE CONTINUOUS QUERY for non-aggregate SELECT stmts with multiple values
		{
			...
			stmt: &sp.CreateContinuousQueryStatement{
				...
				Source: &sp.SelectStatement{
					IsRawQuery: true,
					Fields: []*sp.Field{{Expr: &sp.VarRef{Val: "transmit_rx"}},
						{Expr: &sp.VarRef{Val: "transmit_tx"}}},
					Target: &sp.Target{
						Measurement: &sp.Measurement{RetentionPolicy: "policy1", Name: "network", IsTarget: true},
					},
					Sources: []sp.Source{&sp.Measurement{Name: "myseries"}},
				},
			},
		...
		// CREATE CONTINUOUS QUERY with backreference measurement name
		{
			...
			stmt: &sp.CreateContinuousQueryStatement{
				...
				Database: "testdb",
				Source: &sp.SelectStatement{
					Fields: []*sp.Field{{Expr: &sp.Call{Name: "mean", Args: []sp.Expr{&sp.VarRef{Val: "value"}}}}},
					Target: &sp.Target{
						Measurement: &sp.Measurement{RetentionPolicy: "policy1", IsTarget: true},
					},
					Sources: []sp.Source{&sp.Measurement{Regex: &sp.RegexLiteral{Val: regexp.MustCompile(`^[a-z]+.*`)}}},
					Dimensions: []*sp.Dimension{
						{
							Expr: &sp.Call{
								Name: "time",
								Args: []sp.Expr{
									&sp.DurationLiteral{Val: 1 * time.Minute},
								},
							},
						},
					},
		...
		// CREATE DATABASE statement
		{
			...
			stmt: &sp.CreateDatabaseStatement{
				Name: "testdb",
		...
		// SET PASSWORD FOR USER
		{
			...
			stmt: &sp.SetPasswordUserStatement{
				Name:     "testuser",
				Password: "pwd1337",
			},
		},
		...
		// DROP CONTINUOUS QUERY statement
		{
			...
			stmt: &sp.DropContinuousQueryStatement{Name: "myquery", Database: "foo"},
		},

		// DROP DATABASE statement
		{
			s: `DROP DATABASE testdb`,
			stmt: &sp.DropDatabaseStatement{
				Name: "testdb",
			},
		},
		...
		// DROP MEASUREMENT statement
		{
			...
			stmt: &sp.DropMeasurementStatement{Name: "cpu"},
		},

		// DROP RETENTION POLICY
		{
			s: `DROP RETENTION POLICY "1h.cpu" ON mydb`,
			stmt: &sp.DropRetentionPolicyStatement{
				Name:     `1h.cpu`,
				Database: `mydb`,
			},
		},
		...
		{
			s:    `DROP USER jdoe`,
			stmt: &sp.DropUserStatement{Name: "jdoe"},
		},

		// GRANT READ
		{
			s: `GRANT READ ON testdb TO jdoe`,
			stmt: &sp.GrantStatement{
				Privilege: sp.ReadPrivilege,
				On:        "testdb",
				User:      "jdoe",
			},
		},
		...
		// CREATE RETENTION POLICY
		{
			...
			stmt: &sp.CreateRetentionPolicyStatement{
				Name:        "policy1",
				Database:    "testdb",
				Duration:    time.Hour,
				Replication: 2,
		...
		{
			s:    `SHOW SHARD GROUPS`,
			stmt: &sp.ShowShardGroupsStatement{},
		},

		// SHOW SHARDS
		{
			s:    `SHOW SHARDS`,
			stmt: &sp.ShowShardsStatement{},
		},

		// SHOW DIAGNOSTICS
		{
			s:    `SHOW DIAGNOSTICS`,
			stmt: &sp.ShowDiagnosticsStatement{},
		},
		{
		...
		// CREATE SUBSCRIPTION
		{
			...
			stmt: &sp.CreateSubscriptionStatement{
				Name:            "name",
				Database:        "db",
				RetentionPolicy: "rp",
		...
		// DROP SUBSCRIPTION
		{
			...
			stmt: &sp.DropSubscriptionStatement{
				Name:            "name",
				Database:        "db",
				RetentionPolicy: "rp",
			},
		...
		// SHOW SUBSCRIPTIONS
		{
			...
			stmt: &sp.ShowSubscriptionsStatement{},
		},
	...
	for i, tt := range tests {
		...
		if s, ok := tt.stmt.(*sp.SelectStatement); ok {
			s.GroupByInterval()
		} else if st, ok := stmt.(*sp.CreateContinuousQueryStatement); ok { // if it's a CQ, there is a non-exported field that gets memoized during parsing that needs to be set
			if st != nil && st.Source != nil {
				tt.stmt.(*sp.CreateContinuousQueryStatement).Source.GroupByInterval()
			}
		}
		...
		if !reflect.DeepEqual(tt.err, errstring(err)) {
			...
			if !reflect.DeepEqual(tt.stmt, stmt) {
				...
				switch stmt.(type) {
				case *sp.CreateUserStatement, *sp.SetPasswordUserStatement:
					continue
				}
...
func TestParser_ParseExpr(t *testing.T) {
	var tests = []struct {
		...
		{s: `my_ident`, expr: &sp.VarRef{Val: "my_ident"}},
		{s: `'2000-01-01 00:00:00'`, expr: &sp.StringLiteral{Val: "2000-01-01 00:00:00"}},
		{s: `'2000-01-01'`, expr: &sp.StringLiteral{Val: "2000-01-01"}},
			...
			expr: &sp.BinaryExpr{
				...
				LHS: &sp.BinaryExpr{
					...
					RHS: &sp.BinaryExpr{
						Op:  sp.SUB,
						LHS: &sp.Call{Name: "now"},
						RHS: &sp.DurationLiteral{Val: mustParseDuration("1d")},
					},
				},
				RHS: &sp.BinaryExpr{
					Op:  sp.LT,
...
func newAlterRetentionPolicyStatement(name string, DB string, d, sd time.Duration, replication int, dfault bool) *sp.AlterRetentionPolicyStatement {
	stmt := &sp.AlterRetentionPolicyStatement{
		Name:     name,
		Database: DB,
		Default:  dfault,
	}
