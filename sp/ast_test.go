...
func BenchmarkQuery_String(b *testing.B) {
	p := sp.NewParser(strings.NewReader(`SELECT foo AS zoo, a AS b FROM bar WHERE value > 10 AND q = 'hello'`))
...
func TestInspectDataType(t *testing.T) {
	for i, tt := range []struct {
		...
		{float64(100), sp.Float},
		{int64(100), sp.Integer},
		{int32(100), sp.Integer},
		{100, sp.Integer},
		{true, sp.Boolean},
		{"string", sp.String},
		{time.Now(), sp.Time},
		{time.Second, sp.Duration},
		{nil, sp.Unknown},
	} {
		if typ := sp.InspectDataType(tt.v); tt.typ != typ {
...
func TestSelectStatement_GroupByInterval(t *testing.T) {
	...
	stmt, err := sp.NewParser(strings.NewReader(q)).ParseStatement()
	if err != nil {
		t.Fatalf("invalid statement: %q: %s", stmt, err)
	}

	s := stmt.(*sp.SelectStatement)
...
func TestSelectStatement_SetTimeRange(t *testing.T) {
	...
	hasWhere := false
	sp.WalkFunc(s.Condition, func(n sp.Node) {
		if ex, ok := n.(*sp.BinaryExpr); ok {
			if lhs, ok := ex.LHS.(*sp.VarRef); ok {
				if lhs.Val == "foo" {
					if rhs, ok := ex.RHS.(*sp.StringLiteral); ok {
						if rhs.Val == "bar" {
							hasWhere = true
						}
					}
...
func TestSelectStatement_HasWildcard(t *testing.T) {
	...
	for i, tt := range tests {
		// Parse statement.
		stmt, err := sp.NewParser(strings.NewReader(tt.stmt)).ParseStatement()
		if err != nil {
			t.Fatalf("invalid statement: %q: %s", tt.stmt, err)
		}

		// Test wildcard detection.
		if w := stmt.(*sp.SelectStatement).HasWildcard(); tt.wildcard != w {
...
func TestSelectStatement_RewriteFields(t *testing.T) {
	...
	for i, tt := range tests {
		...
		ic.FieldDimensionsFn = func(sources sp.Sources) (fields map[string]sp.DataType, dimensions map[string]struct{}, err error) {
			source := sources[0].(*sp.Measurement)
			switch source.Name {
			case "cpu":
				fields = map[string]sp.DataType{
					"value1": sp.Float,
					"value2": sp.Integer,
				}
			case "strings":
		...
		// Rewrite statement.
		rw, err := stmt.(*sp.SelectStatement).RewriteFields(&ic)
		if err != nil {
...
func TestSelectStatement_RewriteRegexConditions(t *testing.T) {
	...
	for i, test := range tests {
		stmt, err := sp.NewParser(strings.NewReader(test.in)).ParseStatement()
		if err != nil {
			t.Fatalf("[Example %d], %v", i, err)
		}

		// Rewrite any supported regex conditions.
		stmt.(*sp.SelectStatement).RewriteRegexConditions()

		// Get the expected rewritten statement.
		expStmt, err := sp.NewParser(strings.NewReader(test.out)).ParseStatement()
		if err != nil {
			t.Fatalf("[Example %d], %v", i, err)
		}
...
func TestSelectStatement_RewriteTimeFields(t *testing.T) {
	...
	for i, tt := range tests {
		// Parse statement.
		stmt, err := sp.NewParser(strings.NewReader(tt.s)).ParseStatement()
		if err != nil {
			t.Fatalf("invalid statement: %q: %s", tt.s, err)
		}

		// Rewrite statement.
		stmt.(*sp.SelectStatement).RewriteTimeFields()
...
func TestSelectStatement_HasDerivative(t *testing.T) {
	...
	for i, tt := range tests {
		...
		if d := stmt.(*sp.SelectStatement).HasDerivative(); tt.derivative != d {
...
func TestSelectStatement_IsSimpleDerivative(t *testing.T) {
	...
	for i, tt := range tests {
		...
		// Test derivative detection.
		if d := stmt.(*sp.SelectStatement).IsSimpleDerivative(); tt.derivative != d {
			t.Errorf("%d. %q: unexpected derivative detection:\n\nexp=%v\n\ngot=%v\n\n", i, tt.stmt, tt.derivative, d)
			continue
		}
	}
...
func TestBinaryExprName(t *testing.T) {
	for i, tt := range []struct {
		...
		expr := sp.MustParseExpr(tt.expr)
		switch expr := expr.(type) {
		case *sp.BinaryExpr:
			name := sp.BinaryExprName(expr)
			if name != tt.name {
...
func TestTimeRange(t *testing.T) {
	for i, tt := range []struct {
		...
		{expr: `time > "2000-01-01 00:00:00"`, min: `0001-01-01T00:00:00Z`, max: `0001-01-01T00:00:00Z`, err: `invalid operation: time and *sp.VarRef are not compatible`},
...
func TestOnlyTimeExpr(t *testing.T) {
	...
	for i, tt := range tests {
		...
		if sp.OnlyTimeExpr(stmt.(*sp.SelectStatement).Condition) != tt.exp {
...
func TestRewrite(t *testing.T) {
	...
	act := sp.RewriteFunc(expr, func(n sp.Node) sp.Node {
		switch n := n.(type) {
		case *sp.BinaryExpr:
			return &sp.BinaryExpr{Op: n.Op, LHS: n.RHS, RHS: n.LHS}
		default:
			return n
		}
	})
...
func TestRewriteExpr(t *testing.T) {
	...
	act := sp.RewriteExpr(expr, func(e sp.Expr) sp.Expr {
		switch e := e.(type) {
		case *sp.BinaryExpr:
			if lhs, ok := e.LHS.(*sp.VarRef); ok && lhs.Val == "time" {
				return nil
			}
		}
		return e
...
func TestParseString(t *testing.T) {
	...
	for _, tt := range tests {
		...
		stmtCopy, err := sp.NewParser(strings.NewReader(stmt.String())).ParseStatement()
		if err != nil {
...
func TestEval(t *testing.T) {
	for i, tt := range []struct {
		...
		// Evaluate expression.
		out := sp.Eval(MustParseExpr(tt.in), tt.data)
...
func TestReduce(t *testing.T) {
	...
	for i, tt := range []struct {
		...
		// Fold expression.
		expr := sp.Reduce(MustParseExpr(tt.in), tt.data)
...
func Test_fieldsNames(t *testing.T) {
	for _, test := range []struct {
		...
		for _, s := range test.in {
			...
			fields = append(fields, &sp.Field{Expr: expr})
		}
		got := fields.Names()
	...
	for i, tt := range []struct {
		...
		},
		{
			stmt: &sp.SelectStatement{
				Fields: sp.Fields([]*sp.Field{
					{Expr: &sp.VarRef{Val: "value"}},
					{Expr: &sp.VarRef{Val: "total"}, Alias: "value"},
					{Expr: &sp.VarRef{Val: "value"}},
				}),
			},
...
func TestSelect_Privileges(t *testing.T) {
	stmt := &sp.SelectStatement{
		Target: &sp.Target{
			Measurement: &sp.Measurement{Database: "db2"},
		},
		Sources: []sp.Source{
			&sp.Measurement{Database: "db0"},
			&sp.Measurement{Database: "db1"},
		},
	}

	exp := sp.ExecutionPrivileges{
		sp.ExecutionPrivilege{Name: "db0", Privilege: sp.ReadPrivilege},
		sp.ExecutionPrivilege{Name: "db1", Privilege: sp.ReadPrivilege},
		sp.ExecutionPrivilege{Name: "db2", Privilege: sp.WritePrivilege},
	}
...
func MustTimeRange(expr sp.Expr) (min, max time.Time) {
	min, max, err := sp.TimeRange(expr)
	if err != nil {
		panic(err)
	}
